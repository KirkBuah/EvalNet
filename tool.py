...
from topogen import validate_hammingmesh as vhm
...
if __name__ == "__main__":
    ...
    # HammingMesh
    parser_generate_hammingmesh = parser_generate_subparser.add_parser("hammingmesh", help='generates a board mesh topology')
    parser_generate_hammingmesh.add_argument('n', type=int, help='number of minimum endpoints')
    parser_generate_hammingmesh.add_argument('k', type=int, help='ratio between nodes in the boards and boards')
    def create_hammingmesh_generator(n, k, **kwargs):
        return tg.BoardMesh(
            N=n,
            k=k
        ).generate()

    parser_generate_hammingmesh.set_defaults(func=create_hammingmesh_generator)
    topology_generator_parsers.append(parser_generate_hammingmesh)
    ...
    parser_generate_slimfly.add_argument('q', type=int, help='specifies the size of Galois field (q:=4w + delta where delta = -1 or 0 or 1 and q a prime power)')
    ...
    parser_generate_delorme.add_argument('q', type=int, help='specifies the size of Galois field (q: size of Galois field, q:= 2^(2*a-1), where a = 1,2,3,... and q an odd power of 2)')
    ...
    parser_generate_brown.add_argument('q', type=int, help='specifies the size of Galois field (q: size of Galois field, is a prime power')
    ...
    # Brown Extensions
    parser_generate_brown_ext = parser_generate_subparser.add_parser('brown_ext', help='generates incremental expansions of a Brown topology')
    parser_generate_brown_ext.add_argument('q', type=int, help='specifies the size of Galois field (q: size of Galois field, is a prime power)')
    ...
    # HammingMesh
    parser_validate_hypercube = parser_validate_subparser.add_parser("hammingmesh", help='validates random HammingMesh topologies')
    parser_validate_hypercube.set_defaults(func=vhm.validate)
